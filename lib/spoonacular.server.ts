...
// Server-only: never import this module from a client component, or the key
// would be bundled for the browser. Client code goes through /api/spoonacular.
...
export async function fetchSpoonacularRecipeById(id: string): Promise<SpoonacularRecipeDetails | null> {
  if (!API_KEY || !/^\d{1,10}$/.test(id)) return null;
  try {
    const res = await fetch(
